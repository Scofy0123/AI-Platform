...
4. 用首位管理员飞书账号打开 [http://127.0.0.1:5173/admin/accounts](http://127.0.0.1:5173/admin/accounts)。
...
```text
.data/runtime/codex-accounts/codex-primary
```
...
- SSE 在 Session 到期时主动关闭，并在心跳时重新校验 Session；被撤销的既有连接不会无限继续接收任务事件。
- 项目、任务、事件、审批都按飞书用户 owner 过滤。
- `threadId + turnId` 精确绑定当前 Actor；已结束/伪造 Turn 不能重放 Tool。
- Tool 审计只保存输入/输出摘要哈希和关联 ID，不保存原始飞书 Token。
- 账号管理 API 返回别名、状态、额度和占用，不返回 `CODEX_HOME`。
...
- SQLite 内部审计保留租约、Thread、Turn、Tool 和审批关联；管理员浏览器接口只返回用户、账号别名、任务、动作、结果、摘要和时间，不返回这些运行时关联 ID。
...
任务事件先写入 SQLite，再推送到浏览器。刷新任务页后，客户端用最后一个 sequence 重连，API 根据 `Last-Event-ID` 重放缺失事件。浏览器断线不会主动释放租约。

### API 正常重启

1. 先停止新任务，等待正在运行的只读任务结束。
2. 用 `Ctrl-C` 正常停止，保留 `.data/codexplatform.sqlite` 和整个 `.data/runtime`。
3. 重新执行 `pnpm dev`。
...
5. 检查账号状态、任务时间线和审计记录。对已有 `threadId` 的任务点击“继续”时，real adapter 会尝试 `thread/resume` 并从新的 Turn 边界继续。
...
正常协议终态不会混入恢复态：`TURN_COMPLETED`、`TURN_FAILED`、`TURN_INTERRUPTED` 分别落为 `COMPLETED`、`FAILED`、`INTERRUPTED`。同一任务任一时刻最多存在一个 `ALLOCATING / QUEUED / RUNNING` Turn；重复提交返回 HTTP 409。
...
4. 修复 App Server 或认证问题后，由管理员恢复账号；在原任务点击“继续”，从新的 Turn 边界显式续跑。若无法安全确认前一 Turn 的结果，则保留时间线作为证据并创建新任务。
5. 不要自动重复任何已发出的外部写操作。

这不是生产级自动恢复：当前没有管理员强制回收接口，也不会自动恢复正在执行的 Turn。进入组织试点前，应增加独立 Worker、进程级健康检查和可审计的回收/恢复操作。
...
还要单独备份 `.data/runtime`，但账号凭证目录属于敏感材料，备份必须加密并限制访问。恢复时使用成对的数据库和 Runtime 快照，避免任务 Thread 与账号目录不一致。
...
6. **恢复仍需人工确认。** 已有启动即恢复、15 秒活动 Turn 心跳、crash 后账号隔离、Turn 槽释放和显式 Turn 边界续跑，但没有执行中 Turn 的自动重放；这是为了避免重复外部副作用。
